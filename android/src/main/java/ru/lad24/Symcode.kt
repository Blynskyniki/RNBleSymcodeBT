...
class Symcode(app: Application) {
  companion object {
    private const val TAG = "BLE-Symcode"
    const val NOTIFY_SERVICE_ID = 40
  ...
  // Notify callback
  interface OnNotifyEnabledResult {
  ...
  fun enableNotify(
    resultCb: OnNotifyEnabledResult,
    notifyCb: (message: String) -> Unit
  ) {
    val services = BleManager.getInstance().getBluetoothGatt(this.notifyDevice).services
    val notifyService = services.find { it.instanceId == NOTIFY_SERVICE_ID }
      ...
      BleManager.getInstance().notify(
        ...
        object : BleNotifyCallback() {
          ...
          override fun onNotifyFailure(exception: BleException) {
            log("onNotifyFailure")
            resultCb.result(java.lang.Exception(exception.description))
          }
